        ...
        switch($category){
            case 1:
                case 2:
                    ...
                    $raw = ($source) ? 'count(search_id)' : 'count(review_id) as review_count, truncate(avg(review_point),1)' ;
                    ...
                    ->get();
                    if($category == 1){
                        $favorite_data = DB::table($table.$end)
                        ->select(DB::raw('
                            if(
                                greatest(users.user_sweet,users.user_salty,users.user_hot,users.user_sour,users.user_bitter) = users.user_sweet  
                            , "단 맛",
                            if(
                                greatest(users.user_sweet,users.user_salty,users.user_hot,users.user_sour,users.user_bitter) = users.user_salty
                            , "짠 맛",
                            if(
                                greatest(users.user_sweet,users.user_salty,users.user_hot,users.user_sour,users.user_bitter) = users.user_hot
                            , "매운 맛",
                            if(
                                greatest(users.user_sweet,users.user_salty,users.user_hot,users.user_sour,users.user_bitter) = users.user_sour
                            , "신 맛","쓴 맛")))) as favorite, 
                            count(
                                if(
                                    greatest(users.user_sweet,users.user_salty,users.user_hot,users.user_sour,users.user_bitter) = users.user_sweet  
                                ...
                                if(
                                    greatest(users.user_sweet,users.user_salty,users.user_hot,users.user_sour,users.user_bitter) = users.user_sour
                                , "신 맛","쓴 맛"))))
                            ) as favorite_count, users.user_sex'))
                        ->leftJoin('users','users.user_id','=',$table.$end.'.user_id')
                        ->groupBy(DB::raw('favorite'),'users.user_sex')
                        ->where([['food_id',$food_id],[$table.'_date','>=',$sdate],[$table.'_date','<=',$edate]])
                        ->get();
                    }
